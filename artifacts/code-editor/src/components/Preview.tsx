...
// âââ Markdown â HTML (sem dependÃªncias externas) ââââââââââââââââââââââââââ
...
// âââ JSON â HTML colorido ââââââââââââââââââââââââââââââââââââââââââââââââââ
function jsonToHtml(raw: string): string {
  try {
    ...
    return `<div class="error">JSON invÃ¡lido</div><pre>${raw.replace(/</g, "&lt;")}</pre>`;
  }
}

// âââ Monta HTML completo inlining CSS/JS referenciados ââââââââââââââââââââ
...
// âââ CSS Preview âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function buildCssPreview(css: string): string {
  ...
  <div class="container">
    <h1>PrÃ©via CSS</h1>
    <p>Este Ã© um parÃ¡grafo de exemplo para visualizar seus estilos.</p>
    <button class="btn">BotÃ£o de exemplo</button>
    <a href="#" class="link">Link de exemplo</a>
    <div class="card"><p>Card de conteÃºdo</p></div>
    <ul><li>Item um</li><li>Item dois</li><li>Item trÃªs</li></ul>
...
// âââ JS/TS execution preview ââââââââââââââââââââââââââââââââââââââââââââââ
function buildJsRunner(code: string): string {
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  function addLine(type,args){
    ...
    d.innerHTML='<span class="pfx">'+{log:'â¶',error:'â',warn:'â '}[type]+'</span><span class="val">'+args.map(fmt).join(' ')+'</span>';
    out.appendChild(d);
  }
  ...
  window.onerror=(msg,_,l,c)=>{addLine('err',['Erro na linha '+l+':'+c+' â '+msg]);return true};
  try{
    ${safe}
  }catch(e){addLine('err',[e.toString()])}
  if(count===0) empty.textContent='Nenhuma saÃ­da. Use console.log() para ver resultados aqui.';
  </script></body></html>`;
}
...
export default function Preview({ vfs, activeFile, openFullscreen, onFullscreenOpened, serverUrl, serverPort }: PreviewProps) {
  ...
  const mode: RenderMode = useMemo(() => {
    // Servidor rodando â prioridade mÃ¡xima
  ...
  return (
    <>
    {/* ââ Modal Tela Cheia âââââââââââââââââââââââââââââââââââââââââââ */}
    {showFullscreen && (
        ...
        <div className="flex-1 overflow-auto flex justify-center bg-white">
          ...
          {!hasContent && (
            <div className="flex flex-col items-center justify-center h-full gap-4 bg-[#141c0d]">
              ...
              <p className="text-gray-500 text-base">Sem conteÃºdo para visualizar</p>
    ...
    <div className="h-full flex flex-col bg-[#141c0d]">
      ...
      <div className="flex-1 overflow-auto">
        ...
        {/* Modo Servidor Node.js â iframe proxy */}
        {mode === "server" && serverUrl && (
        ...
        {mode === "none" && (
          <div className="flex flex-col items-center justify-center h-full text-center px-8 gap-3">
            ...
            <p className="text-sm font-medium text-gray-500">Preview nÃ£o disponÃ­vel</p>
            <p className="text-xs text-gray-700 leading-relaxed">
              Tipos suportados: <span className="text-gray-500">HTML Â· JSON Â· Markdown Â· CSS Â· JS/TS</span>
            </p>
            <div className="mt-2 px-4 py-3 bg-green-900/20 border border-green-700/30 rounded-xl max-w-xs text-left">
              <p className="text-[11px] text-green-400 font-bold mb-1">ð¡ Para ver apps Node.js aqui:</p>
              <p className="text-[11px] text-green-700 leading-relaxed">
                ...
                O preview abrirÃ¡ automaticamente quando o servidor iniciar.
              </p>
            </div>
          </div>
        )}
